...
def reduce(image):
  ...
  imagecv=scipy.signal.convolve2d(image, kernel, 'same')
  h,w=imagecv.shape
  imagecv1=imagecv[:,0:w:2]
  imagecv2=imagecv1[0:h:2,:]
  return imagecv2
  # END OF FUNCTION.
...
def expand(image):
  ...
  for i in range(0,h):
   for j in range(0,w):
     x1[2*i,2*j]=image[i,j]
    
  kernel=generatingKernel(0.4)
  imagecv2=scipy.signal.convolve2d(x1, kernel, 'same')
  return 4*imagecv2
...
def laplPyramid(gaussPyr):
  ...
  for i in range(0,len(gaussPyr)-1):
    explayer=expand(gaussPyr[i+1])
    h,w=gaussPyr[i].shape
    if explayer.shape!=(h,w):
      explayer=explayer[0:h,0:w]    
    laplayer=gaussPyr[i]-explayer

    output.append(laplayer)
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  blended_pyr = []
  # WRITE YOUR CODE HERE.
  for k in range(0,len(laplPyrWhite)):
    white_image=laplPyrWhite[k]
    black_image=laplPyrBlack[k]
    current_mask=gaussPyrMask[k]
    bldly=np.zeros(white_image.shape)
    for i in range(0,white_image.shape[0]):
      for j in range(0,white_image.shape[1]):
        bldly[i,j]=current_mask[i,j]*white_image[i,j]+(1-current_mask[i,j])*black_image[i,j]
    blended_pyr.append(bldly)
  return blended_pyr
  # END OF FUNCTION.
...
def collapse(pyramid):
  ...
  for i in range(len(pyramid)-1,0,-1):
    pyramidexp=expand(pyramid[i])
    h,w=pyramid[i-1].shape
    if pyramidexp.shape!=(h,w):
      pyramidexp=pyramidexp[0:h,0:w]
    pyramid[i-1]=pyramid[i-1]+pyramidexp
